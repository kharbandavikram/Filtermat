<?php 
	...
	class Admin extends CI_Controller {
		...
		public function addcategory(){
			...
			$this->form_validation->set_rules('fr_category',"French Category","trim|required|xss_clean");
		...
		public function addsubcategory(){
			...
			$this->form_validation->set_rules('status',"Status","trim|xss_clean|required");
			$this->form_validation->set_rules('fr_subcategory',"French subcategory","trim|required|xss_clean");
		...
		public function editcategory($id=''){
			...
			if($id==''){
				...
				$this->form_validation->set_rules('fr_category',"French Category","trim|xss_clean|required");
		...
		public function editinnercategory($id=''){
			...
			if($id==''){
				...
				$this->form_validation->set_rules('status',"Status","trim|xss_clean|required");
				$this->form_validation->set_rules('fr_innercategory',"French innercategory","trim|xss_clean|required");
		...
		public function addproduct(){
			...
			$this->form_validation->set_rules('status',"Status","trim|xss_clean|required");
			$this->form_validation->set_rules('fr_product_name',"French Product Name","trim|required|xss_clean");
		...
		public function editproduct($id=''){
				...
				if($_FILES){
					...
					if(!in_array($extension,$allowExt) ) {
						...
						if (move_uploaded_file($sourcePath, $targetPath)) {
							...
							$this->admin_model->upload_image($upload_path1.$filename . '.' . $extension);
						} else {
							$data['error'] = "Image not uploaded";
						}
					}
				}
				$data['all_images'] = $this->admin_model->upload_all_image();
			...
			public function addpage(){
				...
				$this->form_validation->set_rules('language_id',"Language","trim|required|xss_clean");
				$this->form_validation->set_rules('page_title',"Title","trim|required|xss_clean");
				//$this->form_validation->set_rules('page_about_desc',"About Description","trim|required|xss_clean");
				//$this->form_validation->set_rules('page_search_for_filters',"Search For Filters","trim|required|xss_clean");
				//$this->form_validation->set_rules('page_realisations',"Realisations Description","trim|required|xss_clean");
				$this->form_validation->set_rules('status',"Status","trim|xss_clean|required");
				if($this->form_validation->run()==false){
					$active['leftbar_active']='pages';
					$this->load->view('admin/includes/header');
					$this->load->view('admin/includes/leftbar',$active);
					$this->load->view('admin/addpage');
					}else{
					
					$data= $this->admin_model->add_page();			
					$this->session->set_flashdata('product_add','Added Successfully');
					redirect(admin_url().'addpage');			
				}
			}
			...
			if($id==''){
				...
				}else if($this->db->where('id',$id)->get('tbl_pages')->num_rows()==0){
				redirect(admin_url().'pages');
				}else{
				$data['page_data']=$this->admin_model->single_page($id);
				$this->form_validation->set_rules('status',"Status","trim|xss_clean|required");
				// $this->form_validation->set_rules('language_id',"Language","trim|xss_clean|required");
				$this->form_validation->set_rules('page_title',"Title","trim|xss_clean|required");
				//$this->form_validation->set_rules('page_about_desc',"About Description","trim|required|xss_clean");
			//	$this->form_validation->set_rules('page_search_for_filters',"Search For Filters","trim|required|xss_clean");
				//$this->form_validation->set_rules('page_realisations',"Realisations Description","trim|required|xss_clean");
				...
				if($this->form_validation->run()==false){
					...
					$data= $this->admin_model->edit_page($id);	
					$this->session->set_flashdata('page_edit','Edited Successfully');
					redirect(admin_url().'editpage/'.$id);	
				}
			}
		}
			public function pages(){
			$this->authAdLogin();
			$data['allpages']=$this->admin_model->pages();
			$active['leftbar_active']='pages';
			$this->load->view('admin/includes/header');
			$this->load->view('admin/includes/leftbar',$active);
		...
		$active_cntry=array('status'=>1);
		$this->db->where('id',$id);
		$this->db->update('tbl_pages',$active_cntry);  
		} 
		public function deactive_page(){
		$this->authAdLogin();
		$id=$this->input->post('id');
		$deactive_cntry=array('status'=>0);
		$this->db->where('id',$id);
		$this->db->update('tbl_pages',$deactive_cntry); 
		}		
		public function deleteupload($id=''){
			$this->authAdLogin();
			if($id==''){
				redirect(admin_url().'uploads');		
				}elseif($this->db->where('id',$id)->get('tbl_upload_images')->num_rows==0){
				redirect(admin_url().'uploads');
				}else{
				$this->db->where('id',$id);
				$this->db->delete('tbl_upload_images'); 
				redirect(admin_url().'uploads');
			}
		}
		...
		public function deletepage($id=''){
			...
			if($id==''){
				redirect(admin_url().'pages');		
				}elseif($this->db->where('id',$id)->get('tbl_pages')->num_rows==0){
				redirect(admin_url().'pages');
				}else{
				$this->db->where('id',$id);
