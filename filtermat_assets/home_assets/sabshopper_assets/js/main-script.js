 ...
 $(document).ready(function () {
     $(".quantity-adder .add-action").click(function () {
         ...
         if ($(this).hasClass('add-up')) {
            var qu= $(ctrl).val(parseInt($(ctrl).val()) + 1);
...
$(document).ready(function(){
	...
	$('.rating.readonly-rating').raty({ 
		...
		readOnly: true,
	    path:"http://sabshopper.sabonclick.com/sabon_assets/home_assets/sabshopper_assets/js/img",
		score: function() {
		...
		if(error == false){	
			...
			$.ajax({
				...
				success: function(data){
				   ...
				   $('.contact_msg').html('<span>'+data+'</span>');
				}
			});
		}
